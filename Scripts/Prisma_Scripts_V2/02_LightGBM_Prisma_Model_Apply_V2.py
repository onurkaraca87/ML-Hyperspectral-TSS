# -*- coding: utf-8 -*-
"""
PRISMA TSS mapping — inference with a pre-trained LightGBM model.
...
MODEL_PATH  = "outputs/LightGBM_v1/lgbm_model.pkl"

TOP10_TXT   = "outputs/LightGBM_v1/top10_bands_LGBM.txt"

PRISMA_TIF  = "data/prisma_scene.tif"

OUTPUT_DIR  = "outputs/LightGBM_v1/model_results"
...
def map_bands_to_prisma(features, sensor_wl, out_dir=None):
    ...
    if out_dir is not None:
        ...
        fig.text(0.5, 0.97, 'Spectral Band Matching: ASD to PRISMA VNIR (LightGBM)',
        ...
        save_path = os.path.join(out_dir, 'band_matching_table_LGBM.png')
...
def assess_radiometric_accuracy(matchup_csv, features, out_dir):
    """
    Assess radiometric accuracy of PRISMA reflectance vs ASD.
...
def main():
    ...
    out_path = out / "PRISMA_LGBM_TSS_Prediction.tif"
    ...
    if n_valid > 0:
        summary_rows = [
            ...
            ["Output file",        "PRISMA_LGBM_TSS_Prediction.tif"],
        ]
        ...
        ax2.set_title("PRISMA LightGBM TSS Prediction — Run Summary",
                      fontsize=11, fontweight='bold', pad=12)
        plt.tight_layout()
        sum_path = os.path.join(str(out), 'run_summary_table_LGBM.png')
    ...
    # ------------------------------------------------------------------
    logging.info("=== LightGBM Model Transfer Summary ===")
