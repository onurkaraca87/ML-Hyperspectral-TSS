# -*- coding: utf-8 -*-
"""
PRISMA TSS mapping — inference with a pre-trained Random Forest model.
...
MODEL_PATH  = "outputs/RandomForest_v4/rf_model.pkl"

TOP10_TXT   = "outputs/RandomForest_v4/top10_bands_RF.txt"

PRISMA_TIF  = "data/prisma_scene.tif"

OUTPUT_DIR  = "outputs/RandomForest_v4/model_results"

# Matchup file: in situ ASD Rrs + PRISMA pixel Rrs at same locations
# Columns required: lat, lon, + band columns (X_400, X_401 ... X_900)
# for both ASD (prefix: asd_) and PRISMA (prefix: prisma_)
# Leave as None to skip radiometric accuracy assessment
MATCHUP_CSV  = None   # e.g. "data/matchups_asd_prisma.csv"
...
def map_bands_to_prisma(features, sensor_wl, out_dir=None):
    ...
    if out_dir is not None:
        ...
        fig.text(0.5, 0.97,
                 'Spectral Band Matching: ASD to PRISMA VNIR (Random Forest)',
                 ha='center', va='top',
        ...
        save_path = os.path.join(out_dir, 'band_matching_table.png')
...
def assess_radiometric_accuracy(matchup_csv, features, out_dir):
    """
    Assess the radiometric accuracy of the hyperspectral reflectance.

    Compares PRISMA-derived Rrs with concurrent ASD Rrs measurements at
    matchup stations. Reports R², RMSE, MAE, and Bias per band and overall.

    Requires matchup CSV with columns:
        asd_X_630, asd_X_677, ...  (ASD Rrs at top-10 wavelengths)
        prisma_X_630, prisma_X_677, ...  (PRISMA Rrs at same pixels)
    """
    ...
    if missing:
        logging.warning(f"Missing columns in matchup CSV: {missing}")
        return
    ...
    r, _ = pearsonr(asd_vals[valid], prisma_vals[valid])

    logging.info(f"  Overall (all bands pooled, n={valid.sum()}):")
    logging.info(f"    R²   = {r2:.4f}")
    logging.info(f"    RMSE = {rmse:.6f} sr⁻¹")
    logging.info(f"    MAE  = {mae:.6f} sr⁻¹")
    logging.info(f"    Bias = {bias:.6f} sr⁻¹")

    # Per-band stats
    logging.info("  Per-band statistics:")
    per_band = []
    for feat, ac, pc in zip(features, asd_cols, prisma_cols):
        ...
        logging.info(f"    {feat}: R²={rb:.4f}  RMSE={per_band[-1]['RMSE']:.6f}  Bias={per_band[-1]['Bias']:.6f}")

    # Scatter plot: PRISMA vs ASD Rrs
    fig, ax = plt.subplots(figsize=(6, 6))
    ax.scatter(asd_vals[valid], prisma_vals[valid],
               c='#2E86C1', s=20, alpha=0.6, zorder=3)
    ...
    if per_band:
        pd.DataFrame(per_band).to_csv(
            os.path.join(out_dir, 'radiometric_accuracy_per_band.csv'),
            index=False)
...
def main():
    ...
    # ------------------------------------------------------------------
    # 4. Radiometric accuracy assessment (if matchup data available)
    # ------------------------------------------------------------------
    assess_radiometric_accuracy(MATCHUP_CSV, features, str(out))

    # ------------------------------------------------------------------
    # 5. Read PRISMA L2D imagery
    #    Atmospheric correction: ASI/MODTRAN (Level-2D product)
    #    No additional atmospheric correction applied.
    #    Model transfer: spectral convolution during training ensures
    #    direct transferability without recalibration.
    # ------------------------------------------------------------------
    ...
    with rasterio.open(PRISMA_TIF) as src:
        ...
        for bnum, feat in zip(band_indices, features):
            ...
            # PRISMA L2D DN → surface reflectance (scale factor = 0.0001)
            if np.nanmax(arr) > 10:
                arr *= 0.0001
            bands_list.append(arr)

        stacked = np.stack(bands_list, axis=-1)   # (H, W, 10)
    ...
    n_total    = pixels.shape[0]  # total pixel count
    valid_mask = (np.all(np.isfinite(pixels), axis=1) &
                  np.any(pixels > 0, axis=1))
    n_valid    = int(valid_mask.sum())
    logging.info(f"Valid pixels: {n_valid:,} / {pixels.shape[0]:,} "
                 f"({100*n_valid/pixels.shape[0]:.1f}%)")
    ...
    out_path = out / "PRISMA_RF_TSS_Prediction.tif"
    ...
    # ------------------------------------------------------------------
    # 8b. Run summary table (printed to console + saved as figure)
    # ------------------------------------------------------------------
    if n_valid > 0:
        summary_rows = [
            ...
            ["Output file",       "PRISMA_RF_TSS_Prediction.tif"],
        ]
        ...
        ax2.set_title("PRISMA RF TSS Prediction — Run Summary",
                      fontsize=11, fontweight='bold', pad=12)
        plt.tight_layout()
        sum_path = os.path.join(str(out), 'run_summary_table.png')
        plt.savefig(sum_path, dpi=1200, bbox_inches='tight')
        plt.close()
        logging.info(f"Run summary table saved → {sum_path}")

    # ------------------------------------------------------------------
    # 8c. False color composite (RGB) of PRISMA image
    #     Red   ~ 660 nm  -> PRISMA Band #28
    #     Green ~ 550 nm  -> PRISMA Band #16
    #     Blue  ~ 480 nm  -> PRISMA Band #9
    # ------------------------------------------------------------------
    logging.info("Generating false color composite (Red/Green/Blue)...")
    try:
        with rasterio.open(PRISMA_TIF) as src:
    ...
    # ------------------------------------------------------------------
    logging.info("=== Model Transfer Summary ===")
    logging.info("  ASD → PRISMA transfer method : spectral convolution")
    logging.info("  Band matching                : argmin(|λ_ASD − λ_PRISMA|)")
    logging.info("  Recalibration required       : No")
    logging.info("  Rationale: Model was trained on ASD spectra convolved")
    logging.info("  to PRISMA band centers. The same convolution is applied")
    logging.info("  here, ensuring input consistency between training and")
    logging.info("  inference without coefficient recalibration.")
    logging.info("Done.")
