# -*- coding: utf-8 -*-
"""
Random Forest regression pipeline for Total Suspended Solids (TSS) retrieval
...
def save_scatter(y_tr, yp_tr, y_te, yp_te, m_tr, m_te,
    ...
    ax.set_title('Random Forest  --  Predicted vs. Observed TSS\n(Combined Dataset)',fontsize=13,fontweight='bold')
...
def save_feature_importance(fi_norm, wl_arr, top10_wl_set, save_path):
    ...
    ax.set_title('Random Forest Feature Importance \u2014 All Bands\n(Red = Top 10)',fontsize=13,fontweight='bold')
    ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)
    plt.tight_layout(); plt.savefig(save_path,dpi=1300,bbox_inches='tight'); plt.close()
...
def save_spectral_region_pie(top10_bands, fi, save_path):
    ...
    ax.set_title(MODEL_NAME+' Top 10 Bands - Spectral Region Distribution (slice size = cumulative feature importance)',
...
def save_repeated_splits_figure(r2_list, rmse_list, save_path):
    ...
    fig.suptitle(f'Random Forest Uncertainty Analysis \u2014 {len(r2_list)} Repeated Random Splits',fontsize=13,fontweight='bold')
    ax=axes[0]
...
def save_validation_figure(spatial_results, temp_result, save_path):
    ...
    fig.suptitle('Random Forest Validation Strategy Comparison\n(Spatial Blocking & Temporal Blocking)',
...
def main():
    ...
    logging.info("Feature selection (initial RF)...")
    rf_fs = RandomForestRegressor(n_estimators=200, random_state=RANDOM_STATE, n_jobs=-1)
    rf_fs.fit(X_full, y)
    fi          = pd.Series(rf_fs.feature_importances_, index=X_full.columns)
    fi_norm     = fi / fi.max()
    ...
    top10_path = out / f'top10_bands_RF_{tag}.txt'
    with open(top10_path, 'w') as f:
        f.write("Top 10 Selected Bands — Random Forest\n")
        f.write("=" * 42 + "\n")
    ...
    save_feature_importance(fi_norm.values, wl_arr, top10_wl, out/f'feature_importance_RF_{tag}.png')
    ...
    joblib.dump(best_rf, out/f'rf_model_{tag}.pkl')
    logging.info(f"Best params: {search.best_params_}")

    # STEP 5 — 5-Fold CV on TRAIN only
    logging.info("5-fold CV on train data...")
    cv_r2   = cross_val_score(best_rf, X_tr, y_tr, cv=kf, scoring='r2')
    cv_rmse = -cross_val_score(best_rf, X_tr, y_tr, cv=kf, scoring='neg_root_mean_squared_error')
    ...
    yp_tr = best_rf.predict(X_tr)
    yp_te = best_rf.predict(X_te)
    ...
    RF_PARAMS = dict(
        ...
        max_depth         = search.best_params_.get('max_depth',           6),
        min_samples_split = search.best_params_.get('min_samples_split',  10),
        min_samples_leaf  = search.best_params_.get('min_samples_leaf',    3),
    ...
    save_repeated_splits_figure(r2_list, rmse_list, out/f'uncertainty_repeated_splits_RF_{tag}.png')
    ...
    for train_loc,test_loc in [('Matagorda','Trinity'),('Trinity','Matagorda')]:
        ...
        rf_b.fit(df.loc[tr_idx,top10_bands], df.loc[tr_idx,'TSS'].astype(float))
        yp_b=rf_b.predict(df.loc[te_idx,top10_bands])
    ...
    rf_t.fit(df.loc[tr_idx_t,top10_bands], df.loc[tr_idx_t,'TSS'].astype(float))
    yp_t=rf_t.predict(df.loc[te_idx_t,top10_bands])
    ...
    save_validation_figure(spatial_results, temp_result, out/f'validation_spatial_temporal_{tag}.png')

    # STEP 12 — Pie chart
    save_spectral_region_pie(top10_bands, fi, out/f'spectral_region_pie_RF_{tag}.png')
    ...
    rpt_path = out / f'model_report_RF_{tag}.txt'
    with open(rpt_path, 'w', encoding='utf-8') as f:
        f.write(f"Random Forest - Model Report\n{'='*42}\n")
        f.write(f"Date      : {tag}\n")
        ...
        f.write(f"      Random Forest: std={pred_std:.3f}, corr={pearson:.4f}\n")
