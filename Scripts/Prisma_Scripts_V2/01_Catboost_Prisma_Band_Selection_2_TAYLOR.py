...
Author:      Onur Karaca
-------------------------------------------------------------------------------
Project:     CatBoost TSS Pipeline — Log Transform + AntiOverfit_2
             Taylor Diagram Statistics added to report
-------------------------------------------------------------------------------
"""
...
OUTPUT_DIR       = r"D:\TWDB\FEEDBACK\ML_Process_Main\Prisma_Process\Catboost_Prisma_2"
...
def save_scatter(y_tr, yp_tr, y_te, yp_te, m_tr, m_te,
    ...
    ax.plot(xf, slope*xf+intercept, '-', color='#E8604C', lw=1.5, zorder=2,
            label=f'Test fit  (slope={slope:.2f})')
...
def save_feature_importance(fi_norm, wl_arr, top10_wl_set, save_path):
    ...
    for wv in sorted(top10_wl_set):
        ...
        if len(idx):
            ax.text(wv, fi_norm[idx[0]]+0.02, str(wv), ha='center', va='bottom',
...
def save_spectral_region_pie(top10_bands, fi, save_path):
    ...
    wedges,texts,autotexts = ax.pie(sizes, colors=colors, explode=explode,
...
def save_repeated_splits_figure(r2_list, rmse_list, save_path):
    ...
    fig.suptitle(f'CatBoost Uncertainty Analysis \u2014 {len(r2_list)} Repeated Random Splits', fontsize=13, fontweight='bold')
    ax = axes[0]
...
def main():
    ...
    cb_fs = CatBoostRegressor(iterations=400, depth=4, learning_rate=0.05,
                               loss_function='RMSE', random_seed=RANDOM_STATE, verbose=0)
    cb_fs.fit(X_full, y_log)
    ...
    save_feature_importance(fi_norm.values, wl_arr, top10_wl, out/f'feature_importance_CB_{tag}.png')
    ...
    search = RandomizedSearchCV(
        CatBoostRegressor(loss_function='RMSE', random_seed=RANDOM_STATE,
    ...
    search.fit(X_tr, y_tr_log, eval_set=(X_te, y_te_log))
    ...
    cv_rmse = -cross_val_score(best_cb, X_tr, y_tr_log, cv=kf, scoring='neg_root_mean_squared_error')
    ...
    obs_std  = float(np.std(y_te.values))
    ...
    for seed in range(N_RANDOM_SPLITS):
        ...
        cb_s.fit(X_tr_s, np.log1p(y_tr_s))
        yp_s = np.expm1(cb_s.predict(X_te_s))
    ...
    save_repeated_splits_figure(r2_list, rmse_list, out/f'uncertainty_repeated_splits_CB_{tag}.png')
    ...
    for train_loc,test_loc in [('Matagorda','Trinity'),('Trinity','Matagorda')]:
        ...
        y_tr_b=df.loc[tr_idx,'TSS'].astype(float); y_te_b=df.loc[te_idx,'TSS'].astype(float)
        cb_b=CatBoostRegressor(**CB_PARAMS)
        cb_b.fit(df.loc[tr_idx,top10_bands], np.log1p(y_tr_b))
        yp_b=np.expm1(cb_b.predict(df.loc[te_idx,top10_bands]))
    ...
    y_tr_t=df.loc[tr_idx_t,'TSS'].astype(float); y_te_t=df.loc[te_idx_t,'TSS'].astype(float)
    cb_t=CatBoostRegressor(**CB_PARAMS)
    cb_t.fit(df.loc[tr_idx_t,top10_bands], np.log1p(y_tr_t))
    yp_t=np.expm1(cb_t.predict(df.loc[te_idx_t,top10_bands]))
    ...
    save_validation_figure(spatial_results, temp_result, out/f'validation_spatial_temporal_CB_{tag}.png')

    # STEP 12 — Pie chart
    save_spectral_region_pie(top10_bands, fi, out/f'spectral_region_pie_CB_{tag}.png')
    ...
    with open(rpt_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"\n  --> Taylor Diagram koduna yaz:\n")
        f.write(f"      REF_STD = {obs_std:.3f}\n")
        f.write(f"      CatBoost: std={pred_std:.3f}, corr={pearson:.4f}\n")
