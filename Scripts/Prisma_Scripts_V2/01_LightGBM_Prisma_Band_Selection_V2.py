# -*- coding: utf-8 -*-
"""
LightGBM regression pipeline for Total Suspended Solids (TSS) retrieval
...
def save_scatter(y_tr, yp_tr, y_te, yp_te,
    ...
    ax.set_title('LightGBM  --  Predicted vs. Observed TSS\n(Combined Dataset)',
...
def save_feature_importance(fi_norm, wl_arr, top10_wl_set, save_path):
    ...
    ax.set_title('LightGBM Feature Importance \u2014 All Bands\n(Red = Top 10)',
                 fontsize=13, fontweight='bold')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    plt.tight_layout()
...
def save_spectral_region_pie(top10_bands, fi, save_path):
    """
    Pie chart: top-10 bands by spectral region.
    Blue 400-499 nm | Green 500-599 nm | Red 600-699 nm | NIR 700-900 nm
    ...
    ax.set_title(' LightGBM Top 10 Bands - Spectral Region Distribution'
                 ' (slice size = cumulative feature importance)',
                 fontsize=11, fontweight='bold', pad=14)
    plt.tight_layout()
    plt.savefig(save_path, dpi=1300, bbox_inches='tight')
    plt.close()
    logging.info('Spectral region pie chart saved to {}'.format(save_path))    
...
def save_repeated_splits_figure(r2_list, rmse_list, save_path):
    ...
    fig.suptitle(f'LightGBM Uncertainty Analysis \u2014 {len(r2_list)} Repeated Random Splits',
                 fontsize=13, fontweight='bold')
    ax = axes[0]
...
def save_validation_figure(spatial_results, temp_result, save_path):
    ...
    fig.suptitle('LightGBM Validation Strategy Comparison\n(Spatial Blocking & Temporal Blocking)',
...
def main():
    ...
    # ------------------------------------------------------------------
    # STEP 2 — Feature selection via LightGBM importance
    # ------------------------------------------------------------------
    logging.info("Feature selection (initial LightGBM)...")
    lgbm_fs = LGBMRegressor(
        n_estimators=500, max_depth=4, learning_rate=0.05,
        num_leaves=15, reg_lambda=10,
        random_state=RANDOM_STATE, n_jobs=-1, verbose=-1)
    lgbm_fs.fit(X_full, y)
    fi          = pd.Series(lgbm_fs.feature_importances_, index=X_full.columns)
    fi_norm     = fi / fi.max()
    ...
    top10_path = out / f'top10_bands_LGBM_{tag}.txt'
    with open(top10_path, 'w') as f:
        f.write("Top 10 Selected Bands — LightGBM\n")
        f.write("=" * 42 + "\n")
    ...
    save_feature_importance(fi_norm.values, wl_arr, top10_wl,
                            out / f'feature_importance_LGBM_{tag}.png')
    ...
    # ------------------------------------------------------------------
    logging.info("LightGBM hyperparameter search...")
    kf = KFold(CV_SPLITS, shuffle=True, random_state=RANDOM_STATE)
    # num_leaves = 2^max_depth - 1 (typical rule); kept small to prevent overfitting
    # reg_lambda increased for regularization on small dataset (n=117)

    
    param_dist = {
    'n_estimators':      [500, 800, 1000, 1500],
    'max_depth':         [3, 4, 5],
    'num_leaves':        [7, 15, 20],   # 31 removed
    'learning_rate':     [0.01, 0.03, 0.05],
    'reg_lambda':        [10, 15, 20, 30],  # daha agresif
    'min_child_samples': [10, 15, 20],  # 5 removed
    }
    ...
    joblib.dump(best_lgbm, out / f'lgbm_model_{tag}.pkl')
    ...
    cv_r2   = cross_val_score(best_lgbm, X_tr, y_tr, cv=kf, scoring='r2')
    cv_rmse = -cross_val_score(best_lgbm, X_tr, y_tr, cv=kf,
                                scoring='neg_root_mean_squared_error')
    ...
    # ------------------------------------------------------------------
    yp_tr = best_lgbm.predict(X_tr)
    yp_te = best_lgbm.predict(X_te)
    ...
    # ── TAYLOR DIAGRAM STATS (full dataset) ────────────────────────────────
    yp_all   = best_lgbm.predict(X_top)
    ...
    LGBM_PARAMS = dict(
        ...
        num_leaves       = search.best_params_.get('num_leaves',          15),
        learning_rate    = search.best_params_.get('learning_rate',     0.05),
        reg_lambda       = search.best_params_.get('reg_lambda',          10),
        min_child_samples= search.best_params_.get('min_child_samples',   10),
    ...
    for seed in range(N_RANDOM_SPLITS):
        ...
        lgbm_s = LGBMRegressor(**LGBM_PARAMS)
        lgbm_s.fit(X_tr_s, y_tr_s)
        yp_s   = lgbm_s.predict(X_te_s)
    ...
    save_repeated_splits_figure(r2_list, rmse_list,
                                out / f'uncertainty_repeated_splits_LGBM_{tag}.png')
    ...
    for train_loc, test_loc in [('Matagorda', 'Trinity'), ('Trinity', 'Matagorda')]:
        ...
        lgbm_b = LGBMRegressor(**LGBM_PARAMS)
        lgbm_b.fit(df.loc[tr_idx, top10_bands], df.loc[tr_idx, 'TSS'].astype(float))
        yp_b   = lgbm_b.predict(df.loc[te_idx, top10_bands])
    ...
    lgbm_t   = LGBMRegressor(**LGBM_PARAMS)
    lgbm_t.fit(df.loc[tr_idx_t, top10_bands], df.loc[tr_idx_t, 'TSS'].astype(float))
    yp_t     = lgbm_t.predict(df.loc[te_idx_t, top10_bands])
    ...
    save_validation_figure(spatial_results, temp_result,
                           out / f'validation_spatial_temporal_LGBM_{tag}.png')

    # ------------------------------------------------------------------
    # STEP 12 — Spectral region pie chart
    # ------------------------------------------------------------------
    save_spectral_region_pie(top10_bands, fi,
                             out / f'spectral_region_pie_LGBM_{tag}.png')
    ...
    # STEP 14 — Full report
    # ------------------------------------------------------------------
    rpt_path = out / f'model_report_LGBM_{tag}.txt'
    with open(rpt_path, 'w', encoding='utf-8') as f:
        f.write(f"LightGBM - Model Report\n{'='*42}\n")
        f.write(f"Date      : {tag}\n")
        ...
        f.write(f"\nTaylor Diagram Statistics (Full Dataset)\n{'-'*25}\n")
        ...
        f.write(f"      LightGBM: std={pred_std:.3f}, corr={pearson:.4f}\n")
